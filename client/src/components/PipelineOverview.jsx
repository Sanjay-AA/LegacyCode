...
export default function PipelineOverview({ hasAnalysis = false, hasPlan = false, hasMigrated = false, hasVerified = false, hasShipped = false, activeTab = 'analyze' }) {
  return (
    <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
        {PIPELINE_STAGES.map((stage) => {
          ...
          const isDone = isAnalyzeDone || isPlanDone || isMigrateDone || isVerifyDone || isShipDone;
          const isActive = stage.id === activeTab ||
            ...
            (stage.id === 'ship' && hasVerified && !hasShipped);
